...
# Define the step interval for saving the model
save_interval = 5  # Save the model every 5 epochs
# Define learning rate adjustment strategy
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=8, gamma=0.1)  # Reduce learning rate every 8 epochs
...
# Track the number of epochs with no improvement
patience_counter = 0
...
if latest_checkpoint_path:
    ...
    print(f"Latest checkpoint loaded: {latest_checkpoint_path}, starting from epoch {start_epoch}.")
else:
...
if os.path.exists(log_filename):
    ...
    if not log_data.empty:
        ...
        print(f"Best test accuracy read from log file: {best_test_accuracy:.4f}")
else:
...
def compute_metrics(all_targets, all_predictions):
    ...
    try:
        ...
        auc = float('nan')  # Prevent errors when only one class is in the sample
    try:
        auprc = average_precision_score(all_targets, all_predictions, average='macro')
    except ValueError:
        auprc = float('nan')  # Handle errors like above
...
for epoch in range(start_epoch, num_epochs):
    ...
    # Check for a new best performance
    if test_accuracy > best_test_accuracy:
        ...
        patience_counter += 1  # Increment the no-improvement counter

    # Check if the early stopping threshold is reached
